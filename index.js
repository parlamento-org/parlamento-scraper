...
} from "./pdf_propostas.js";
import proposal_links from "./proposals_links.json" with { type: "json" };
...
function determineProposalResult(
  votacaoGeneralidadeObj,
  votacaoEspecialidadeObj
) {
    ...
    fetch(pdfURL, {
      ...
      .then((response) => {
        response.arrayBuffer().then((buffer) => {
          ...
          console.log(
            "Writing downloaded PDF file to " + outputFilename + "..."
          );
...
function generateVotingBlocks(votacaoObj) {
  ...
  if (votacaoAFavor.length > 0)
    votacaoAFavor = votacaoAFavor[0]["string"].split(",");

  let votacaoContra = splits.filter((split) => split["vote"] === "Contra");
  if (votacaoContra.length > 0)
    votacaoContra = votacaoContra[0]["string"].split(",");

  let votacaoAbstencao = splits.filter(
    (split) => split["vote"] === "Abstenção"
  );
  if (votacaoAbstencao.length > 0)
    votacaoAbstencao = votacaoAbstencao[0]["string"].split(",");
  ...
  votingBlocks.push(
    ...votingBlockByOrientation("Abstaining", votacaoAbstencao)
  );
...
async function processProjetosLei(projetosLei, forbidden_words) {
  ...
  for (let i = 0; i < projetosLei.length; i++) {
    ...
    console.log(
      `--- PROCESSING(${i + 1} / ${projetosLei.length}) : ` +
        projetoLei["IniTitulo"]
    );
    ...
    const votacaoGeneral = projetoLei["IniEventos"].filter((evento) => evento["CodigoFase"] === "250");
    let votacaoGeneralObj = null;
    if (votacaoGeneral.length > 0) {
      if (votacaoGeneral[votacaoGeneral.length - 1]["Votacao"])
        votacaoGeneralObj =
          votacaoGeneral[votacaoGeneral.length - 1]["Votacao"][0];
      else {
        console.log(
          "--------  Warning: Found votacao generalidade mas nenhuma Votacao -------------"
        );
        console.log(votacaoGeneral);
      }
    }

    const votacaoEspecialidade = projetoLei["IniEventos"].filter((evento) => evento["CodigoFase"] === "320");
    let votacaoEspecialidadeObj = null;

    if (votacaoEspecialidade.length > 0) {
      if (
        votacaoEspecialidade[votacaoEspecialidade.length - 1]["Votacao"]
      )
        votacaoEspecialidadeObj =
          votacaoEspecialidade[votacaoEspecialidade.length - 1]["Votacao"][0];
      else {
        console.log(
          "--------  Warning: Found votacao especialidade but no Votacao -------------"
        );
    ...
    const proposalResult = determineProposalResult(
      votacaoGeneralObj,
      votacaoEspecialidadeObj
    );
    ...
    await sleep(5000);
    convertPDFtoHTML(proposalLink, outputFilename, forbidden_words)
      .then((html) => {
        ...
        fetch(`http://0.0.0.0:8080/proposal/`, {
          ...
          .then((response) => {
            if (!response.ok) {
              ...
              console.log(
                `Proposal ${sourceId} stored in the database successfully!`
              );
            }
          })
          .catch((error) => {
...
async function processLegislature(leg_link, leg_arg) {
  ...
  //filter projetosLei for iniciativas where the array["iniEventos"] have an element with codigoFase == '250'

  projetosLei = projetosLei.filter(
    (iniciativa) =>
      Array.isArray(
        iniciativa["IniEventos"]
      ) &&
